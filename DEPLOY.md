...
# GitHub에서 새 리포 만든 후 (예: beautyinforties)
git remote add origin https://github.com/<YOUR_USER>/beautyinforties.git
git branch -M main
...
3. 다음 값 입력:
   - **Name**: `beautyinforties-backend` (subdomain이 됨 → `beautyinforties-backend.onrender.com`)
...
6. 완료되면 `https://beautyinforties-backend.onrender.com` 접속 → `/health` 가 `{"status":"ok"}` 반환하면 OK
...
4. **Environment Variables** 추가:
   - `NEXT_PUBLIC_API_BASE` = `https://beautyinforties-backend.onrender.com`  (1번 단계 URL)
5. **Deploy** → 1~3분 대기
...
1. Render 대시보드 → beautyinforties-backend → **Environment**
